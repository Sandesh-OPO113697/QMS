    ...
    protected void CreateAccountBySuper(object sender, EventArgs e)
{
    if (Page.IsValid)
    {
        ...
        if (selectedProcesses.Count > 0 && users.SelectedIndex >0)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                con.Open();
                foreach (string selectedProcess in selectedProcesses)
                {
                    ...
                    if (!string.IsNullOrEmpty(processName))
                    {
                        ...
                        if (existingCount == 0)
                        {
                            ...
                            Notification.InnerText = "Program Assign Sucessfully !";
                        }
                    }
                }
            }
        ...
        {
            Response.Write("<script>alert('Please select at least one process and User');</script>");
        }
    }
    //Response.Redirect("ListProcess", false);
}
